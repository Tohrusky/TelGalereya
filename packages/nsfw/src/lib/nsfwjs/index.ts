...
export class NSFWJS {
  public endpoints: string[] | undefined
  public model: tf.LayersModel | tf.GraphModel | undefined

  private readonly options: nsfwjsOptions
  private readonly pathOrIOHandler: string | IOHandler
  private intermediateModels: { [layerName: string]: tf.LayersModel } = {}
  private readonly normalizationOffset: tf.Scalar
  ...
  async load() {
    ...
    if (type === 'graph') {
      ...
      this.endpoints = this.model.layers.map((l: { name: any }) => l.name)
    }
